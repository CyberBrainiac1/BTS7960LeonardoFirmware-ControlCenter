...
void InitPWM() {
  ...
  PWM16EnableD();
#endif // end of 2 ffb axis
#elif defined(__AVR_ATmega168__)|| defined(__AVR_ATmega328P__) || defined(__AVR_ATmega32U4__)
...
#ifndef USE_MCP4725
#if defined(ARDUINO_ARCH_RP2040)
static uint32_t calcPwmFreqHz(byte state, uint16_t top) {
  uint32_t base = 16000000UL; // match original AVR frequency table (16 MHz)
  if (bitRead(state, 1) && bitRead(state, 6)) {
    base /= 8; // RCM mode used a /8 prescaler on AVR
  }
  if (top == 0) {
    return 0;
  }
  if (bitRead(state, 0)) {
    return base / (2UL * (top + 1UL)); // phase-correct
  }
  return base / (top + 1UL); // fast PWM
}

void PWM16Begin() { // RP2040: configure PWM range/frequency
  uint32_t freq = calcPwmFreqHz(pwmstate, TOP);
  if (freq > 0) {
    analogWriteFreq(freq);
  }
  analogWriteRange(TOP);
...
void PWM16EnableA() { pinModeFast(PWM_PIN_L, OUTPUT); }
void PWM16EnableB() { pinModeFast(PWM_PIN_R, OUTPUT); }
#ifdef USE_TWOFFBAXIS
void PWM16EnableC() { pinModeFast(PWM_PIN_U, OUTPUT); }
void PWM16EnableD() { pinModeFast(PWM_PIN_D, OUTPUT); }
#endif

inline void PWM16A(uint16_t PWMValue) { analogWrite(PWM_PIN_L, constrain(PWMValue, 0, TOP)); }
inline void PWM16B(uint16_t PWMValue) { analogWrite(PWM_PIN_R, constrain(PWMValue, 0, TOP)); }
#ifdef USE_TWOFFBAXIS
inline void PWM16C(uint16_t PWMValue) { analogWrite(PWM_PIN_U, constrain(PWMValue, 0, TOP)); }
inline void PWM16D(uint16_t PWMValue) { analogWrite(PWM_PIN_D, constrain(PWMValue, 0, TOP)); }
#endif
